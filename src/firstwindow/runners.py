...
def agnes_command(project: Path, task_id: str, task: str) -> Sequence[str]:
    ...
    recipe.write_text(json.dumps({
        ...
        "instructions": "Work only inside the current project. Respect AGENTS.md. Run relevant tests/build checks before claiming completion and report concrete evidence.",
        "prompt": task
    }, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return ["agnes", "run", "--recipe", str(recipe)]

def hermes_command(project: Path, task_id: str, task: str, model: str) -> Sequence[str]:
    runtime = project / ".firstwindow" / "runtime"
    runtime.mkdir(parents=True, exist_ok=True)
    usage = runtime / f"{task_id}-hermes-usage.json"
    return ["hermes", "-z", task, "--provider", "custom", "--model", model, "--usage-file", str(usage)]
